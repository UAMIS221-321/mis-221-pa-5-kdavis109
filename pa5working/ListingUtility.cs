...
private void EditListing()
{
    ...
    else
    {
        ...
        Console.Write("Enter new Time of the Session (HH:mm, leave empty to keep current value): ");
        string sessionTimeInput = Console.ReadLine();
        if (!string.IsNullOrEmpty(sessionTimeInput))
        {
            TimeSpan sessionTime;
            while (!TimeSpan.TryParse(sessionTimeInput, out sessionTime))
            {
                System.Console.WriteLine("Invalid input. Please enter a valid time in the format HH:mm.");
